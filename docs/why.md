...
`go-ruby-fast-gettext-locale/fast-gettext-locale` reimplements Ruby's `fast-gettext-locale` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-fast-gettext-locale/fast-gettext-locale` directly, with no Ruby runtime;
...
Because `fast-gettext-locale` is pure Go with cgo disabled, it also compiles to **WebAssembly**
